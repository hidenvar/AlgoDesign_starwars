...
Scenario5::Scenario5(Graph g, Inventory i)
    : Scenario(g), nights(5, false), inventory(i) {}

void Scenario5::solve() { return; }
...
void Scenario5::attack(int nightIdx) {
  ...
  for (const auto& mtypeStr : missilePriority) {
    int& count = [&]() -> int& {
      ...
      if (mtypeStr == "C2") return inventory.C2;
      if (mtypeStr == "D1") return inventory.D1;
      static int dummy = 0;
      return dummy;
    }();
  ...
  if (!nights[nightIdx]) {
    std::cout << "**** Starting fallback attack phase *****\n";
...
Scenario5::MinimalTarget Scenario5::findMostEfficientTarget() {
  ...
  if (fallbackBases.empty()) return best;
...
std::vector<Scenario5::AttackStep> Scenario5::planMinimalAttack(
  ...
  for (const auto& fb : bases) {
    if (missilesPlanned >= missilesNeeded) break;
...
int Scenario5::getInventoryCount(const std::string& type) {
  if (type == "A1") return inventory.A1;
  if (type == "A2") return inventory.A2;
  if (type == "A3") return inventory.A3;
  if (type == "B1") return inventory.B1;
  if (type == "B2") return inventory.B2;
  if (type == "C1") return inventory.C1;
  if (type == "C2") return inventory.C2;
  if (type == "D1") return inventory.D1;
  return 0;
}